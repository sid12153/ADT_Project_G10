...
def get_city_id(city_detail, cur):
    try:
        ...
        if len(city_list) != 3:
            raise ValueError("Invalid city detail format. Expected format: 'City, State, Country'")
        ...
        if rows:
            ...
            raise ValueError("City details not found in database")
    
    except Exception as e:
        raise ValueError(f"Error retrieving city ID: {str(e)}")
...
class Get_Ride_Details_Request_Body(BaseModel):
    ride_id: int
# class Fetch_Ride_Details_Request_Body(BaseModel):
#     ride_id = int
...
@app.post("/get_ride_details", status_code=200)
async def ride_details(ride_details: Get_Ride_Details_Request_Body): #Fetch_Ride_Details_Request_Body
    conn = postgres_connection()
    cur = conn.cursor()
    rider_query = """
    ...
    WHERE ride_id=%s
    """
    cur.execute(rider_query, (ride_details.ride_id,))
    rows = cur.fetchall()
    conn.commit()
    conn.close()
...
@app.post("/add_ride", status_code=200)
async def add_ride(add_rider_details: Add_Ride_Request_Body):
    conn = postgres_connection()
    cur = conn.cursor()
    try:
        # retrieving departure and arrival city ids
        departure_city_id = get_city_id(add_rider_details.departure_city, cur)
        arrival_city_id = get_city_id(add_rider_details.arrival_city, cur)

        # retrieving the currency id
        currency_id = get_currency_id(add_rider_details.currency, cur)

        print("Received add ride request")

        # Executing the SQL insert query for the user
        insert_query = """
        INSERT INTO rides(
            user_id, departure_city_id, arrival_city_id, currency_id, departure_date, departure_time,
            arrival_date, arrival_time, vehicle_type, vehicle_number, pickup_loc, dropoff_loc,
            total_seats, reserved_seats, available_seats, price, special_amenities, active
        ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        RETURNING ride_id;
        """
        cur.execute(insert_query, (
            add_rider_details.user_id, departure_city_id, arrival_city_id, currency_id, add_rider_details.departure_date,
            add_rider_details.departure_time, add_rider_details.arrival_date, add_rider_details.arrival_time,
            add_rider_details.vehicle_type, add_rider_details.vehicle_number,
            add_rider_details.pickup_loc, add_rider_details.dropoff_loc, add_rider_details.available_seats, 0,
            add_rider_details.available_seats, add_rider_details.price, add_rider_details.special_amenities, True
        ))

        ride_id = cur.fetchone()[0]
        conn.commit()

        print(f"Ride added successfully for user:{add_rider_details.user_id} with ride ID:{ride_id}")
        return {"status": "Success", "message": "Ride added successfully", "ride_id": ride_id}

    except Exception as e:
        conn.rollback()
        cur.close()
        conn.close()
        print(f"Failed to add ride: {str(e)}")
        return JSONResponse(
            status_code=500,
            content={"status": "Error", "message": "Failed to add ride: " + str(e)}
        )
    finally:
        # Ensure the cursor and connection are closed properly in every case
        cur.close()
        conn.close()
...
async def update_ride(ride_update_details: Update_Ride_Request_Body):
    ...
    try:
        ...
        for key, value in ride_update_details.dict(exclude={'ride_id'}).items():
            if value is not None:
                column = key + "_id" if key in ["departure_city", "arrival_city", "currency"] else key
                if key in ["departure_city", "arrival_city"]:
                    value = get_city_id(value, cur)
                elif key == "currency":
                    value = get_currency_id(value, cur)

                update_set += f"{column}=%s,"
                value_set.append(value)

        if not update_set:
            return {"status": "Error", "message": "No fields to update"}

        update_query = f"UPDATE rides SET {update_set[:-1]} WHERE ride_id=%s RETURNING ride_id;"
        value_set.append(ride_update_details.ride_id)
        
        print("update ride request")
        print(update_query)
        cur.execute(update_query, tuple(value_set))
        row = cur.fetchone()
        conn.commit()

        if row:
            return {"status": "Success", "message": "Ride updated successfully", "ride_id": row[0]}
        else:
            return {"status": "Error", "message": "No ride found with the given ID"}
...
async def passenger_rides(passenger_details: Fetch_Passenger_Rides_Request_Body):
    ...
    column_names = [
        ...
        "Arrival City", "Arrival State", "Arrival Country", "Departure Date", 
        "Departure Time", "Arrival Date", "Arrival Time", "Vehicle Type",
        "Vehicle Number", "Vehicle Image", "Pickup Location", "Dropoff Location", 
        "Total Seats", "Reserved Seats", "Available Seats", "Price", 
        "Currency", "Special Amenities"
    ]
...
async def update_reservation(reservation_details: Update_Reservation_Request_Body):
    ...
    try:
        ...
        if rows:
            ...
            response = {"status": "Success", "message": "Booking update is successful", "booking_id": returned_booking_id}
        else:
            # If no rows are returned, it means no records were updated
            response = {"status": "Error", "message": "No booking found with the provided booking ID"}
        ...
        conn.rollback()
        print(f"Failed to update booking: {str(e)}")
        return JSONResponse(
            status_code=500,
            content={"status": "Error", "message": "Failed to update booking: " + str(e)}
        )
    finally:
        cur.close()
        conn.close()
...
async def get_cities():
    conn = postgres_connection()
    cur = conn.cursor()
    cur.execute("SELECT city,state,country FROM Cities;")
    cities = cur.fetchall()
    conn.close()
    # Unpack the city names from the query result
    city_names = ["{}, {}, {}".format(entry[0], entry[1], entry[2])  for entry in cities]
    return city_names
...
async def passenger_reservations(user_details: Fetch_Reservation_Request_Body):
    ...
    SELECT bks.ride_id, bks.booking_id, bks.number_of_seats_reserved, 
    ...
    # Define custom column names here
    column_names = [
        "Ride ID", "Booking ID", "Seats Reserved", "Booking Date", "Booking Time",

...
def fetch_rides(user_id):
    response = requests.post(f"{API_URL}/fetch_rides", json={"user_id": user_id})
    return response.json()
...
if 'show_book_ride_form' not in st.session_state:
    st.session_state.show_book_ride_form = False

def toggle_book_ride_form():
...
def show_rider_dashboard():
    ...
    if st.session_state.show_add_ride_form:
        ...
        with st.form("add_ride_form"):
            ...
            currency = st.selectbox("Currency", options=currencies)  # Example currencies
            ...
            if submit_button:
                ...
                if response.get("status") == "Success":
                    ...
                    st.error("Failed to add ride. Try again!!") 
...
if 'search_rides' not in st.session_state:
    st.session_state.search_rides = False

if 'book_ride_data' not in st.session_state:
    st.session_state.book_ride_data = None

def toggle_search_rides_form():
    st.session_state.search_rides = not st.session_state.search_rides
...
def show_passenger_dashboard():
    ...
    if st.button("Search Rides"):
        ...
        st.session_state.book_ride_data = filters
        
    if st.session_state.search_rides:
        rides = fetch_available_rides(st.session_state.book_ride_data)
        if not rides:
            ...
            for ride in rides:
                ...
                with st.expander(ride_info):
                    ...
                    with st.form(key=f"book_ride_form_{ride['Ride ID']}"):
                        st.write(f"Booking Ride ID: {ride['Ride ID']}")
                        seats_to_reserve = st.number_input("Number of seats to reserve", min_value=1, max_value=ride['Available Seats'], key=f"num_seats_{ride['Ride ID']}")
                        submit_button = st.form_submit_button("Complete Reservation")
                        if submit_button:
                            booking_details = {
                                "ride_id": ride['Ride ID'],
                                "passenger_id": st.session_state['user_id'],
                                "number_of_seats_reserved": seats_to_reserve,
                                "price": ride['Price']
                            }
                    ...
                    #     toggle_book_ride_form()
                    #     print(st.session_state.show_book_ride_form)

        # if st.session_state.show_book_ride_form:
        #     print("book ride request.")
        #     book_ride()
...
#     #     booking_details = st.session_state.book_ride_data
#     #     st.write(f"Booking Ride ID: {st.session_state['booking_ride_id']}")
...
#             if book_response.get("status") == "Success":
#                 st.success("Ride booked successfully.")
#                 toggle_book_ride_form()  # Hide form after submission
#                 st.rerun()
